...
class ChatItemDelivery implements IChatItemDelivery {
  ...
  public postAnswer(answer: Answer) {
    ...
    this.globalSocket
      .to(answer.user.roomId)
      .emit("PUB_CHAT_ITEM", answerResponse)
  }

  public pinChatItem(chatItem: ChatItem): void {
    this.globalSocket.to(chatItem.user.roomId).emit("PUB_PINNED_MESSAGE", {
